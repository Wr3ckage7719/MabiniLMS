-- ============================================
-- MabiniLMS Complete Database Schema
...
CREATE INDEX idx_submissions_drive_file ON public.submissions(drive_file_id);
...
-- Validate institutional email (optional enforcement)
CREATE OR REPLACE FUNCTION validate_profile_email()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT is_institutional_email(NEW.email) THEN
        RAISE EXCEPTION 'Only @mabinicolleges.edu.ph email addresses are allowed';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Uncomment to enforce institutional email validation:
