...
export default function TeacherDashboardPage() {
  ...
  const needsGrading = useMemo(
    () => data.recentSubmissions.filter((s) => !s.grade && s.status !== 'draft'),
    ...
    <div className="w-full h-full overflow-auto animate-fade-in">
      <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <Button size="sm" variant="outline" className="rounded-xl gap-2" onClick={() => void refetch()}>
            ...
            <CardContent className="space-y-2">
              {data.recentSubmissions.length === 0 ? (
                ...
                data.recentSubmissions.slice(0, 5).map((submission) => {
                  ...
                  const initials = `${firstName.charAt(0)}${lastName.charAt(0)}`.toUpperCase() || studentName.slice(0, 2).toUpperCase();
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        if (courseId) navigate(`/class/${courseId}?assignmentId=${assignmentId}`);
                      }}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {graded ? (
                          <Badge className={STATUS_BADGE.success}>Graded</Badge>
                        ) : (
                          <Badge className={STATUS_BADGE.warning}>Grade now</Badge>
                        )}
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{formatDateTime(assignment.due_date)}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge className={`${toneClass} text-[10px]`}>{relative.label}</Badge>
                        <Badge variant="secondary" className="text-[10px]">{assignment.max_points} pts</Badge>
                      </div>
                    </button>
                  );
                })
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {data.courses.slice(0, 6).map((course) => (
                  ...
                  >
                    ...
                    {course.section ? (
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">{course.section}</p>
                    ) : null}
  ...
  return (
    ...
    >
      <CardContent className="p-4 md:p-5">
        ...
        {hint ? <p className="text-[11px] mt-1 text-muted-foreground">{hint}</p> : null}
      </CardContent>
    </Card>
  );
}
